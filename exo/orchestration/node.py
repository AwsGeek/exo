...
class Node:
  ...
  async def update_peers(self, wait_for_peers: int = 0) -> bool:
    ...
    async def disconnect_with_timeout(peer, timeout=300):
      try:
    ...
    async def connect_with_timeout(peer, timeout=300):
      try:
  ...
  async def broadcast_result(self, request_id: str, result: List[int], is_finished: bool) -> None:
    ...
    async def send_result_to_peer(peer):
      try:
        await asyncio.wait_for(peer.send_result(request_id, result, is_finished), timeout=600.0)
  ...
  async def broadcast_opaque_status(self, request_id: str, status: str) -> None:
    ...
    async def send_status_to_peer(peer):
      try:
        await asyncio.wait_for(peer.send_opaque_status(request_id, status), timeout=600.0)
      except asyncio.TimeoutError:
